...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for group in GrupoAlumnos.objects.all():
            if group.id_survey_profesor_autoevaluacion and (not group.token_profesor_autoevaluacion):
        ...
        time.sleep(4)
        #create tokens
        grupos = GrupoAlumnos.objects.filter(profesor__autoevaluacion=True, token_profesor_autoevaluacion__exact='')
        for grupo in grupos:
            participant_data = []
